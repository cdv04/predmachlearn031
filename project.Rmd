...
Read more: http://groupware.les.inf.puc-rio.br/har#ixzz3jEzlbcK7

##Loading and Cleaning Data/Exploratory Data Analysis
```{r echo=FALSE}
...
trainPercent <- 0.25
```
`r (trainPercent*100)`% of the training data set was used for the actual training.  The rest was used as our probe data set for checking the accuracy of our model.
```{r echo=FALSE}
...
###Checking the model data against the training and probe data sets
####Training data set
```{r echo=FALSE}
# check train
predictMyTrainSet <- predict(modFit, newdata=myTrainSet)

results <- data.frame(predictVal = predictMyTrainSet, expectedVal = myTrainSet$classe)
results$match <- results$predictVal == results$expectedVal

results %>% group_by(expectedVal, match) %>% summarise(n = n())
```

####Probe data set
```{r echo=FALSE}
# check probe
predictMyProbeSet <- predict(modFit, newdata=myProbeSet)

results <- data.frame(predictVal = predictMyProbeSet, expectedVal = myProbeSet$classe)
results$match <- results$predictVal == results$expectedVal
